...
Disable-ScheduledTask -TaskName 'Parental Device Activity' -ErrorAction SilentlyContinue | Out-Null
...
Write-Host "$ChildUsername is enabled. The blocker service, its kiosk, and watchdog are disabled until a parent re-enables them:"
Write-Host "  Set-Service RowePcBlocker -StartupType Automatic; Start-Service RowePcBlocker"
Write-Host "  Enable-ScheduledTask 'Parental Device Activity'; Enable-ScheduledTask 'Parental Device Activity Watchdog'"
